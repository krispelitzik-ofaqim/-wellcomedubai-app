  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 12, paddingBottom: 60 }}>
        {!q.trim() ? (
          <View style={s.empty}>
            <Text style={s.emptyTitle}>{t('search.emptyTitle')}</Text>
            <Text style={s.emptySub}>{t('search.emptySub')}</Text>
          </View>
        ) : results.length === 0 ? (
          <View style={s.empty}>
            ...
            {results.map(item => (
              <TouchableOpacity key={item._cat + item.id} style={s.row} onPress={() => router.push(`/item/${item.id}?cat=${item._cat}` as any)}>
                ...
                <View style={{ flex: 1, marginRight: 10 }}>
                  ...
                  <Text style={s.cat}>{(t('cat.' + item._cat).startsWith('cat.') ? CAT_NAME[item._cat] : t('cat.' + item._cat))}{item.address ? ' · ' + item.address : ''}</Text>
                </View>
